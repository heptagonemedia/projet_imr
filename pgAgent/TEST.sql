...
DECLARE
    ...
    resultat BOOLEAN; -- pour débugger uniquement.
...
BEGIN
    ...
    profondeur_verification := 3; -- On consulte ces valeurs pour établir la moyenne de l'intervale d'incertitude.
    ...
    FOR i IN 1..profondeur_verification LOOP
        moyenne := moyenne + (
            ...
            AND hdb.date_saisie = temps_present - (i * INTERVAL '1 SECOND')
            AND tdm.etiquette = type_donnee
        ); -- val actu - x
    END LOOP;

    moyenne := moyenne/(profondeur_verification-1);

    -- Calcul de l'écart type
    FOR i IN 1..profondeur_verification LOOP
        ecart_type := POWER(ecart_type +(
            SELECT m.valeur
            FROM historique_donnee_bouee AS hdb
            JOIN mesure m ON hdb.id_historique_donnee_bouee = m.id_historique_donnee_bouee
            JOIN type_donnee_mesuree tdm ON m.id_type_donnee_mesuree = tdm.id_type_donnee_mesuree
    ...
    -- RETURN (NOT (valeur_mesure*0.2 < moyenne OR valeur_mesure*1.8 > moyenne));
    -- Si la valeur est trop petite ou trop grande : FALSE.

    -- PLPGSQL ne gère pas un return direct, je suis obligé de passer par des IF ELSE
    -- C'est pas la vrai formule de l'intervale de confiance, mais c'est assez précis
    -- et le code est déjà très lourd. On a une marge de 5%.
    IF (valeur_mesure*0.05 < moyenne OR valeur_mesure*1.05 > moyenne) THEN
        resultat := FALSE;
    ELSE
        resultat := TRUE;
    END IF;
...
VALUES (1, 1, 800000); -- historique 1 et type 1
